    ...
    public boolean isValidMove(int newMoveX, int newMoveY, Piece[][] board) {
        ...
        if(newMoveX != x){
            ...
            for (int i = posMinX; i <= posMaxX; i++) {
                if(i == posMinX){
                    if(!flagCheckExits && x == posMaxX + 1 && board[posMinX][y] != null){
                ...
                if (i == posMaxX){
                    if(!flagCheckExits && x == posMinX - 1 && board[posMaxX][y] != null){
        ...
        if(newMoveY != y){
            ...
            for (int i = posMinY; i <= posMaxY; i++) {
                if(i == posMinY){
                    if(!flagCheckExits && y == posMaxY + 1 && board[x][posMinY] != null){
                ...
                if (i == posMaxY){
                    if(!flagCheckExits && y == posMinY - 1 && board[x][posMaxY] != null){
